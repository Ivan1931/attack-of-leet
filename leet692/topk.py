...
class Solution(object):
    def topKFrequent(self, words, k):
        ...
        for w in words:
            frequencies[w] += 1
        topk = list(frequencies.items())
        topk.sort(key=lambda item: (-item[1], item[0]))
        topk = topk[0:k]
        return list(map(lambda item: item[0], topk))
